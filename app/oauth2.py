...
def get_user(db : Session , id : int) :
    user_verify = db.query(User).filter(User.id == id).first()
    if not user_verify :
...
def authenticate_user(db :Session , 
                      email:EmailStr,
                      password : str) :
    user_verify = db.query(User).filter(User.email == email).first()
    if not user_verify :
...
def trainer_required(user = Depends(get_current_user)) :
    if user.role != "trainer" :
        raise HTTPException(403,"trainer priviliges required")
    return user

def admin_required(user = Depends(get_current_user)) :
    if user.role != "admin" :
        raise HTTPException(403,"admin priviliges required")
    return user
